...
Test Template    USDTIDR Market Smoke

*** Variables ***
${ENV}    dev

*** Test Cases ***
TC-01 Trader Executes Market Buy Order using Total Available Balance
    [Tags]    web    flow    trade    market    pro    P0    TC-01
    TC-01    ${ENV}
TC-02 Trader Executes Market Sell Order by Specifying Asset Quantity
    [Tags]    web    flow    trade    market    pro    P0    TC-02
    TC-02    ${ENV}
TC-03 Trader Places Limit Buy Order at a Price that Gets Filled
    [Tags]    web    flow    trade    limit    pro    P1    TC-03
    TC-03    ${ENV}
TC-04 Trader Places Limit Sell Order resulting in Partial Execution
    [Tags]    web    flow    trade    limit    pro    P1    TC-04
    TC-04    ${ENV}
TC-05 Trader Manages Risk with Stop-Limit Sell Order to Mitigate Loss
    [Tags]    web    flow    trade    stop_limit    pro    P1    TC-05
    TC-05    ${ENV}
TC-06 Trader Capitalizes on Breakout with Stop-Limit Buy Order
    [Tags]    web    flow    trade    stop_limit    pro    P1    TC-06
    TC-06    ${ENV}
TC-07 Trader is Prohibited from Trading Below Minimum Transaction Limit
    [Tags]    web    flow    trade    negative    validation    pro    P2    TC-07
    TC-07    ${ENV}
TC-08 Trader Releases Frozen Funds by Cancelling Open Order
    [Tags]    web    flow    trade    order_management    pro    P1    TC-08
    TC-08    ${ENV}
